...
custom_model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=5)
# 2 Training Loop
...
custom_model.compile(loss=loss, optimizer=optimizer)
custom_model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=15)
# 3 Training Loop
for layer in base_model.layers:
    layer.trainable = True
# Compile the model
...
custom_model.compile(loss=loss, optimizer=optimizer)
custom_model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=30)
# Save the model
