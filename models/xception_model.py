...
# 2 Training Loop
for layer in xception_custom.layers[10:]:
    layer.trainable = True
# Compile the model
loss = keras.losses.CategoricalCrossentropy()
optimizer = keras.optimizers.SGD(learning_rate=0.01, momentum=0.7)
xception_custom.compile(loss=loss, optimizer=optimizer)
history = xception_custom.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=10)
# 3 Training Loop
...
history = xception_custom.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=10)
# Save the model
xception_custom.save("saved_models/custom_xception.keras")
